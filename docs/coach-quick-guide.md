...
5. **`committed`** roster counts stay at zero until a super admin sets them. You may run **multiple squads** under your name (same age, league, gender). If your next team would repeat the **exact** auto-generated name for that season, confirm the **-Black / -Red** popup: the app renames your existing roster **-Black** and saves the new one as **-Red** (only when the duplicate is **your** roster—not another coach’s).
6. On the **Teams** list, **Roster season** is a **dropdown**: known years come from squads already in the database plus your club env default plus the season you’re currently viewing—it updates immediately when changed (same as other dropdown filters). Use **Apply** after editing **name search** only. Squads **without a league** can be filtered with **No league / internal only**; **Reset filters** returns you to **`/teams`** with the env default roster year. Strange text in the address bar (`seasonLabel=`) is normalized on the server; use **Reset filters** or **/teams** if something looks corrupted.
